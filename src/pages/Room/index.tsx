            ...
            <main>
                ...
                {questions.length === 0 ? (
                        <div className={styles['no-questions']}>
                            ...
                            <span>{`${!user ? 'Faça o seu login e seja' : 'Seja'} a primeira pessoa a fazer uma pergunta!`}</span>
                        </div> 
                    ) : (
                            ...
                            {questions.map(question => {
                                return (
                                    <Question key={question.id} content={question.content} author={question.author} isAnswered={question.isAnswered} isHighlighted={question.isHighlighted}>
                                        {!question.isAnswered && (
                                            <button className={`like-button ${question.likeId ? 'liked' : ''}`} type="button" aria-label="Marcar como gostei" onClick={() => handleLikeQuesiton(question.id, question.likeId)}>
